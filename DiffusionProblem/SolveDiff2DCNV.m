function T1 = SolveDiff2DCNV(T0,Q,rho,dt,P,N,B)
...
switch B.ttbc
    case {'flux','neumann'}
        T0(N.nz,2:(N.nx-1)) = T0(N.nz,2:(N.nx-1)) + 4*sz*N.dz*B.thf + ...
            Q(N.nz,2:(N.nx-1)).*dt./rho(N.nz,2:(N.nx-1))./P.cp;
    case {'const','dirichlet'}
        Q(N.nz,:)           = 0;
end
switch B.btbc
    case {'flux','neumann'}
        T0(1,2:(N.nx-1))    = T0(1,2:(N.nx-1)) - 4*sz*N.dz*B.bhf + ...
            Q(1,2:(N.nx-1)).*dt./rho(1,2:(N.nx-1))./P.cp;
    case {'const','dirichlet'}
        Q(1,:)              = 0;
end
switch B.ltbc
    case {'flux','neumann'}
        T0(2:(N.nz-1),1)    = T0(2:(N.nz-1),1) - 4*sx*N.dx*B.lhf + ...
            Q(2:N.nz-1,1).*dt./rho(2:N.nz-1,1)./P.cp;
    case {'const','dirichlet'}
        Q(2:N.nz-1,1)       = 0;
end
switch B.rtbc
    case {'flux','neumann'}
        T0(2:(N.nz-1),N.nx) = T0(2:(N.nz-1),N.nx) + 4*sx*N.dx*B.rhf + ...
            Q(2:N.nz-1,N.nx).*dt./rho(2:N.nz-1,N.nx)./P.cp;
    case {'const','dirichlet'}
        Q(2:N.nz-1,N.nx)    = 0;
end
