function [T2,N] = SolveDiff2DADI(T0,Q,rho,dt,Py,N,B)
...
a       = Py.kappa*dt/N.dz^2/2;
b       = Py.kappa*dt/N.dx^2/2;
...
switch lower(B.btbc)
    ...
    case {'flux','neumann'}
        bind            = Number1(N.nz,2:N.nx-1);
        bind2           = Number2(N.nz,2:N.nx-1);
        
        % Vertikale Laufrichtung - 1. ADI Schritt
        diag1(N.nz,2)       = 1;        % Left lower Corner Grid point
        diag1(bind-1,1)     = -2*a;     % j,nz-1
        diag1(bind,2)       = 1+2*a;    % j,nz
        diag1(N.nx*N.nz,2)  = 1;        % right lower Corner Grid point
        
        diag2(N.nz,2)       = 1;
        diag2(bind-N.nz,1)  = b;        % j-1,nz
        diag2(bind,2)       = 1-2*b;    % j,nz
        diag2(bind+N.nz,3)  = b;        % j+1,nz
        diag2(N.nx*N.nz,2)  = 1;
        ...
        diag3(bind2-1,1)            = -b;       % j-1,nz
        diag3(bind2,2)              = 1+2*b;    % j,nz
        diag3(bind2+1,3)            = -b;       % j+1,nz
        diag3(N.nx*N.nz,2)          = 1;
        
        diag4(N.nx*(N.nz-1)+1,2)    = 1;
        diag4(bind2-N.nx,1)         = 2*a;      % 1,nz-1
        diag4(bind2,2)              = 1-2*a;    % j,nz
        diag4(N.nx*N.nz,2)          = 1;
...
switch lower(B.ttbc)
    ...
    case {'flux','neumann'}
        tind            = Number1(1,2:N.nx-1);
        tind2           = Number2(1,2:N.nx-1);
        
        % Vertikale Laufrichtung - 1. ADI Schritt
        diag1(1,2)          = 1;        % Left lower Corner Grid point
        diag1(tind,2)       = 1+2*a;    % j,1
        diag1(tind+1,3)     = -2*a;     % j,2
        diag1((N.nx-1)*N.nz+1,2)  = 1;  % right lower Corner Grid point
        
        diag2(1,2)          = 1;
        diag2(tind-N.nz,1)  = b;        % j-1,1
        diag2(tind,2)       = 1-2*b;    % j,1
        diag2(tind+N.nz,3)  = b;        % j+1,1
        ...
        % Horizontale Laufrichtung - 2. ADI Schritt
        diag3(1,2)          = 1;
        diag3(tind2-1,1)    = -b;       % j-1,1
        diag3(tind2,2)      = 1+2*b;    % j,1
        diag3(tind2+1,3)    = -b;       % j+1,1
        diag3(N.nx,2)       = 1;
        
        diag4(1,2)          = 1;
        diag4(tind2,2)      = 1-2*a;    % j,1
        diag4(tind2+N.nx,3) = 2*a;      % j,2
        diag4(N.nx,2)       = 1;
...
rhsQ1   = reshape(Q,[N.nx*N.nz,1]).*dt/2./...
...
rhsQ2   = reshape(Q',[N.nx*N.nz,1]).*dt/2./...
            reshape(rho',[N.nx*N.nz,1])./Py.cp;
