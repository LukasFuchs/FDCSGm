...
if (j==1)||(i==1)||(j==N.nx)||(i==(N.nz1))
    ...
    switch B.tbc
        case 0
            ...
            if(i==1)&&(j>1)&&(j<N.nx)
                ABC(G-1+di(10),10)      = 3*N.Kbond/2;    % vx(i,j)
                % South
                ABC(G-1+di(13),13)      = -N.Kbond/2;     % vx(i+1,j)
            end
        case 1
            % --------------------- FREE SLIP --------------------------- %
            ...
            if(i==1)&&(j>1)&&(j<N.nx)
                % South
                ABC(G-1+di(13),13)      = -N.Kbond;       % vx(i+1,j)
            end
    end
    %% LOWER BOUNDARY --------------------------------------------------- %
    switch B.bbc
        case 0
            ...
            if(i==(N.nz1))&&(j>1)&&(j<N.nx)
                ABC(G-1+di(10),10)    = 3*N.Kbond/2;    % vx(i,j)
                % North
                ABC(G-1+di(7),7)      = -N.Kbond/2;     % vx(i-1,j)
            end
        case 1
            % --------------------- FREE SLIP --------------------------- %
            % ----------- dvx/dz = 0; vx(i,j)-vx(i-1,j)=0 --------------- %
...
if (j==1)||(i==1)||(j==(N.nx-1))||(i==N.nz)    
    ...
    switch B.lbc
        case 0
            ...
            if (j==1)&&(i>1)&&(i<(N.nz))
                ABC(G+di(10),10)  =     3*N.Kbond/2;    % vz(i,j)
                % East
                ABC(G+di(19),19)  =     -N.Kbond/2;     % vz(i,j+1)
            end
        case 1
            % --------------------- FREE SLIP --------------------------- %
