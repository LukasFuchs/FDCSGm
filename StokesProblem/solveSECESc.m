function [D,A] = solveSECESc(D,Py,N,B)
% ----------------------------------------------------------------------- %
...
if (j==1)||(i==1)||(j==N.nx)||(i==(N.nz1))
    ...
    switch B.tbc
        case 0
            ...
            if(i==1)&&(j>1)&&(j<N.nx)
                ABC(G-1+di(19),19)      = 3/2;      % vx(i,j)
                % South
                ABC(G-1+di(22),22)      = -1/2;       % vx(i+1,j)
            end
        case 1
            % --------------------- FREE SLIP --------------------------- %
...
if(i>1)&&(j>1)&&(j<N.nx)&&(i<(N.nz1))
    ...
    %% S1
    ABC(G-1+di(21),21)     = 1/N.dx;          % Pressure at S1
    ...
    ABC(G-1+di(34),34)     = 1/N.dx^2*(eta(i,j)+eta(i,j+1)+eta(i+1,j)+...
        eta(i+1,j+1))/2;                            % Vx
    %% S1E1
    ABC(G-1+di(36),36)     = -1/N.dx;         % Pressure at S1E1
end
end
...
if (j==1)||(i==1)||(j==(N.nx-1))||(i==N.nz)    
    ...
    switch B.lbc
        case 0
            ...
            if (j==1)&&(i>1)&&(i<(N.nz))
                ABC(G+di(19),19)  = 3/2;    % vz(i,j)
                % East
                ABC(G+di(34),34)  = -1/2;     % vz(i,j+1)
            end
        case 1
            % --------------------- FREE SLIP --------------------------- %
...
if(j>1)&&(i>1)&&(j<(N.nx-1))&&(i<N.nz)
    ...
    %% E
    ABC(G+di(32),32)     = 1/N.dz;            % P at E1
    ABC(G+di(34),34)     = eta(i,j+1)/N.dx^2;
    %% S1E1
    ABC(G+di(35),35)     = -1/N.dz;           % P at S1E1
end
end
    ...
    switch B.rbc
        case {0,1}
            if (j==2)&&(i==2) || (j==2)&&(i==N.nz) || (j==3)&&(i==2) || ...
                ...
                %% N1
                ABC(G-2+di(17),17)  =   1/N.dx;       % vx(i-1,j)
            end
    end
end
end
